...
public class PlayerController : MonoBehaviour
{
    ...
    private float intervaloEntreSaltos = 0.2f;
    ...
    private void Start()
    {
        ...
        entradas.Movimiento.Salto.performed += context => SaltoContext(context);
        entradas.Movimiento.Dash.performed += contexti => DashContext(contexti);
    }
    ...
    private void FixedUpdate()
    {
        ...
        // Detectar si está en la pared, pero ignorar plataformas
        Collider2D colisionPared = Physics2D.OverlapBox(controladorPared.position, dimensionesCajaPared, 0f, queEsSuelo);
        if (colisionPared != null && colisionPared.tag == "Plataforma")
        {
            enPared = false; // Ignorar si es plataforma
        }
        else
        {
            enPared = colisionPared != null; // Solo marcar como enPared si no es plataforma
        }
    ...
    private void Mover(float mover, bool saltar)
    {
        ...
        if (saltar)
        {
            if (Time.time - tiempoUltimoSalto > intervaloEntreSaltos) // Asegúrate de que ha pasado suficiente tiempo desde el último salto
            {
                if (enSuelo)
                {
                    Salto();
                    saltosExtraRestantes = saltosExtra;  // Restablecer los saltos cuando tocamos el suelo
                    saltoDoble = false;  // Desactivar salto doble cuando estamos en el suelo
                    animator.SetBool("saltoDoble", false);
                }
                else if (enPared)
                {
                ...
                // Registra el tiempo del último salto
    ...
    private void SaltoContext(InputAction.CallbackContext context)
    {
        salto = true;
    }
    ...
    private void DashContext(InputAction.CallbackContext context)
    {
        Debug.Log("Dash activado");
        if (puedeHacerDash) // Evitar hacer el dash si está en cooldown
        {
            StartCoroutine(Dash());
        }
    }
