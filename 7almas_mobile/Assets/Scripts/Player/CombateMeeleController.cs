...
public class CombateMeeleController : MonoBehaviour
{
    ...
    private void Awake()
    {
        entradas = new Entradas(); // Inicializamos el nuevo sistema de entradas
    }
    ...
    {
        entradas.Disable(); // Desactivamos las entradas cuando el objeto se desactiva
    }

    private void Start()
    {
        animator = GetComponent<Animator>();
        entradas.Gameplay.AtaquePrincipal.performed += context => Golpe(); // Ataque principal
        entradas.Gameplay.AtaqueSecundario.performed += context => GolpeSecundario(); // Ataque secundario
    }
